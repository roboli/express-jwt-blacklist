...
module.exports = function(store) {
  ...
  var url  = store.url;
  // Must be a promise, use it to close connection gracefully
  var quit = store.quit;

  store.options = store.options || {};
  
  var client = store.client || url ? redis.createClient(url, store.options) : redis.createClient(port, host, store.options);
